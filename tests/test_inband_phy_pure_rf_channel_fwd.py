...
class blk(gr.basic_block):
    ...
    def __init__(self, p_drop=0.0):
        gr.basic_block.__init__(self, name='Pure RF Channel (FWD)', in_sig=None, out_sig=None)
        self.p_drop = p_drop
    ...
    def handle_msg(self, msg):
        ...
        if random.random() < self.p_drop:
            print(f"  [PHY DROP FWD] Type={pkt_type:<7} Length={len(raw):>4}B | Seq={seq} (LOST)", flush=True)
            return
        
        print(f"  [PHY TX->RX]   Type={pkt_type:<7} Length={len(raw):>4}B | Src={raw[2]} Dst={raw[4]} Seq={seq} Tot={tot} Sid=0x{sid:08X}", flush=True)

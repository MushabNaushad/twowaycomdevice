...
class qpsk_test(gr.top_block, Qt.QWidget):
    ...
    def __init__(self):
        gr.top_block.__init__(self, "QPSK Test", catch_exceptions=True)
        ...
        self.rrc_taps = rrc_taps = firdes.root_raised_cosine(1, samp_rate, samp_rate/float(sps), excess_bw, sps*nfilts)
        self.qpsk_const = qpsk_const = digital.constellation_rect([-1-1j, -1+1j, 1+1j, 1-1j], [0, 1, 3, 2],
        4, 2, 2, 1, 1).base()
        self.noise_volt = noise_volt = .200
        self.frequency_offset = frequency_offset = .025

        ##################################################
        # Blocks
        ##################################################

        self._time_offset_range = qtgui.Range(.999, 1.001, 0.000001, 1.0005, 200)
        ...
        self._noise_volt_range = qtgui.Range(0, 1, .1, .200, 200)
        self._noise_volt_win = qtgui.RangeWidget(self._noise_volt_range, self.set_noise_volt, "Channel : Voltage Noise", "counter_slider", float, QtCore.Qt.Horizontal)
        self.top_layout.addWidget(self._noise_volt_win)
        self._frequency_offset_range = qtgui.Range(-0.1, 0.1, 0.001, .025, 200)
        ...
        self.connect((self.fft_filter_xxx_0, 0), (self.digital_symbol_sync_xx_0_0, 0))

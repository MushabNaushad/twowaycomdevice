...
run_actual_sweep.py
===================
Executes GNU Radio parameter test for a single (m, mtu) across all drop rates:
  - drop_prob in [0.0, 0.05, 0.15, 0.30, 0.50]
  - 5 simultaneous transmitting node pairs (1->6, 2->7, 3->8, 4->9, 5->10)
  - 512 bytes payload per session

Usage:
  python3 run_actual_sweep.py <m> <mtu>
"""
...
class SweepFlowGraph(gr.top_block, Qt.QWidget):
    def __init__(self, m, mtu, drop_prob=0.0, rto_ms=600, payload_bytes=512, strobe_period_ms=2500):
        gr.top_block.__init__(self, f'Sweep_m{m}_mtu{mtu}', catch_exceptions=True)
        Qt.QWidget.__init__(self)
        self.setWindowTitle(f'Sweep m={m} mtu={mtu}')
        ...
        payload_data = bytes([i % 256 for i in range(payload_bytes)])
        n = len(payload_data)
        
        for src_addr, dst_addr in self.pairs:
            hdr = bytes([dst_addr, 0, 1, 0, (n >> 24) & 0xFF, (n >> 16) & 0xFF, (n >> 8) & 0xFF, n & 0xFF])
            raw = list(hdr + payload_data)
            pdu = pmt.cons(pmt.make_dict(), pmt.init_u8vector(len(raw), raw))
            
            strobe = blocks.message_strobe(pdu, strobe_period_ms)
...
# ─── Runner Function ──────────────────────────────────────────────────────────
def run(m, mtu, drop_values=[0.0, 0.05, 0.15, 0.30, 0.50], phase_duration_s=3.5, payload_bytes=512, rto_ms=600):
    qapp = Qt.QApplication(sys.argv)
    tb = SweepFlowGraph(m=m, mtu=mtu, drop_prob=drop_values[0], rto_ms=rto_ms, payload_bytes=payload_bytes, strobe_period_ms=2500)
    tb.start()
    tb.show()
    ...
    def advance_phase():
        ...
        strobe_fires = max(1, int(elapsed / 2.5))
        expected_injections = 5 * strobe_fires
        success_rate = min(1.0, phase_deliveries / float(expected_injections)) if expected_injections > 0 else 0.0
        ...
        results.append(row)
        print(f"  -> Loss {current_drop*100:>2.0f}%: {phase_deliveries:>2}/{expected_injections} delivered | {total_delivered_bytes/1024:>5.1f} KB | Throughput: {throughput_kbps:>6.2f} kB/s | Success: {success_rate*100:>5.1f}%", flush=True)

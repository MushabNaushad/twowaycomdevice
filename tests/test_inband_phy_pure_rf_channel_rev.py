...
class blk(gr.basic_block):
    ...
    def __init__(self, p_drop=0.0):
        gr.basic_block.__init__(self, name='Pure RF Channel (REV)', in_sig=None, out_sig=None)
        self.p_drop = p_drop
    ...
    def handle_msg(self, msg):
        ...
        if random.random() < self.p_drop:
            print(f"  [PHY DROP REV] Type={pkt_type:<7} Length={len(raw):>4}B | Seq={seq} (LOST)", flush=True)
            return
        
        print(f"  [PHY RX->TX]   Type={pkt_type:<7} Length={len(raw):>4}B | Seq={seq}", flush=True)

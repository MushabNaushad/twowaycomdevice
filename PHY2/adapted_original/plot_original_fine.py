...
# -*- coding: utf-8 -*-
"""
PHY2 Adapted Original Transceiver - Fine Multi-Slice Visualization Engine
Renders high-resolution multi-line parametric slice plots, 2D correlation heatmaps,
and an interactive HTML dashboard for the adapted original transceiver using y·y' TED.
"""
...
def render_fine_analysis_plots(results_dir):
    json_path = os.path.join(results_dir, "fine_original_sweep_results.json")
    ...
    for mod in ['BPSK', 'QPSK']:
        costas_vals = sorted(list(set([r['costas_bw'] for r in results])))[::4] # Sample 5-6 values across 0.005..0.500
        ...
        for cbw in costas_vals:
            ...
            if pts:
                series_1.append({'name': f'Costas BW = {cbw:.3f}', 'points': pts})
                
        svg_p1 = generate_multiline_svg(
            title=f"{mod} (y·y' TED): BER vs Symbol Sync BW (Fixed FLL = {fixed_fll:.4f})",
            x_label="Symbol Synchronizer Loop Bandwidth (rad/sym)",
            y_label="Bit Error Rate (BER)",
            series_data=series_1,
            is_log_y=True
        )
        with open(os.path.join(results_dir, f"chart_original_fixed_fll_ber_vs_symsync_{mod.lower()}.svg"), "w") as f:
            f.write(svg_p1)
    ...
    fixed_costas = 0.0628
    for mod in ['BPSK', 'QPSK']:
        freq_offsets = [-0.030, -0.015, 0.0, +0.015, +0.030]
        ...
        svg_p2 = generate_multiline_svg(
            title=f"{mod} (y·y' TED): BER vs FLL Bandwidth (Fixed Costas = {fixed_costas:.4f})",
            x_label="FLL Band-Edge Loop Bandwidth (rad/sym)",
            y_label="Bit Error Rate (BER)",
            series_data=series_2,
            is_log_y=True
        )
        with open(os.path.join(results_dir, f"chart_original_fixed_costas_ber_vs_fll_{mod.lower()}.svg"), "w") as f:
            f.write(svg_p2)
    ...
    for mod in ['BPSK', 'QPSK']:
        ...
        svg_p3 = generate_multiline_svg(
            title=f"{mod} (y·y' TED): PDR (%) vs Noise Voltage (Fixed Sym BW = {fixed_sym})",
        ...
        with open(os.path.join(results_dir, f"chart_original_fixed_symsync_pdr_vs_noise_{mod.lower()}.svg"), "w") as f:
            f.write(svg_p3)
            
    # Read pinpoint JSON
    pinpoint_json = os.path.join(results_dir, "fine_pinpoint_optimal_parameters.json")
    pinpoint_data = {}
...
<head>
    ...
    <title>PHY2 Adapted Original Transceiver Fine Optimization Dashboard (y·y' TED)</title>
    <style>
        :root {{
            --primary: #2563eb;
            --primary-dark: #1d4ed8;
...
<body>
    <div class="header">
        <h1>PHY2 Adapted Original Transceiver Fine Parameter Optimization</h1>
        <p>Fine-step analysis using TED_SIGNAL_TIMES_SLOPE_ML (y·y' TED), Costas & Symbol Sync BW range 0.005 to 0.500 ({len(results):,} trials)</p>
    </div>

    <div class="container">
        <!-- Pinpointed Optimal Parameters Summary -->
        <div class="card full-width">
            <h2>Optimal Parameter Recommendations (y·y' TED Mode)</h2>
            <div class="opt-grid">
                <div class="opt-box">
                    <h3>BPSK (y·y' TED)</h3>
                    ...
                    <div class="opt-item"><strong>Symbol Sync BW:</strong> 0.0250 rad/sym (TED_SIGNAL_TIMES_SLOPE_ML)</div>
                    <div class="opt-item"><strong>Preamble Length:</strong> 32 Bytes</div>
                    <div class="opt-item"><strong>Average PDR:</strong> 90.0%</div>
                </div>
                <div class="opt-box">
                    <h3>QPSK (y·y' TED)</h3>
                    <div class="opt-item"><strong>FLL Loop BW:</strong> 0.0314 rad/sym</div>
        ...
        <div class="card">
            ...
            {open(os.path.join(results_dir, "chart_original_fixed_fll_ber_vs_symsync_bpsk.svg")).read()}
        </div>

        <div class="card">
            <h2>2. QPSK: BER vs Symbol Sync BW for Varied Costas BW (Fixed FLL = 0.0314)</h2>
            {open(os.path.join(results_dir, "chart_original_fixed_fll_ber_vs_symsync_qpsk.svg")).read()}
        </div>

        <!-- Slice 2: Fixed Costas Slice -->
        <div class="card">
            <h2>3. BPSK: BER vs FLL Bandwidth for Varied Frequency Offsets (Fixed Costas = 0.0628)</h2>
            {open(os.path.join(results_dir, "chart_original_fixed_costas_ber_vs_fll_bpsk.svg")).read()}
        </div>

        <div class="card">
            <h2>4. QPSK: BER vs FLL Bandwidth for Varied Frequency Offsets (Fixed Costas = 0.0628)</h2>
            {open(os.path.join(results_dir, "chart_original_fixed_costas_ber_vs_fll_qpsk.svg")).read()}
        </div>

        <!-- Slice 3: Fixed Symbol Sync Slice -->
        <div class="card">
            <h2>5. BPSK: PDR vs Noise Voltage for Varied Preamble Lengths (Fixed SymSync = 0.045)</h2>
            {open(os.path.join(results_dir, "chart_original_fixed_symsync_pdr_vs_noise_bpsk.svg")).read()}
        </div>

        <div class="card">
            <h2>6. QPSK: PDR vs Noise Voltage for Varied Preamble Lengths (Fixed SymSync = 0.045)</h2>
            {open(os.path.join(results_dir, "chart_original_fixed_symsync_pdr_vs_noise_qpsk.svg")).read()}
        </div>
    </div>
</body>
</html>
"""
    with open(os.path.join(results_dir, "original_dashboard.html"), "w") as f:
        f.write(html_dashboard)
        
    print(f"[OK] Fine multi-slice plots and dashboard generated into: {results_dir}")

if __name__ == '__main__':
    res_dir = os.path.join(os.path.dirname(__file__), "results")
    render_fine_analysis_plots(res_dir)

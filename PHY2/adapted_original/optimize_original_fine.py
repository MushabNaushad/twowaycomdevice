...
# -*- coding: utf-8 -*-
"""
PHY2 Adapted Original Transceiver - Ultra-Fine Deep Parameter Optimizer
Evaluates Costas Loop BW and Symbol Sync Loop BW from 0.005 to 0.500 (step size 0.005)
...
def run_fine_optimization():
    print("================================================================================")
    print("   PHY2 ADAPTED ORIGINAL: ULTRA-FINE DEEP OPTIMIZATION (y·y' TED)               ")
    print("================================================================================")
    ...
    # Costas and Symbol Sync 0.005 to 0.500 with step 0.005 (100 values!)
    costas_dense = [round(x, 4) for x in np.arange(0.005, 0.505, 0.005)]
    sym_dense    = [round(x, 4) for x in np.arange(0.005, 0.505, 0.005)]
    
    # Subsampled high-density arrays for Cartesian combinatorial slices:
    # 20 fine representative values across 0.005 -> 0.500:
    costas_fine = [round(x, 4) for x in np.linspace(0.005, 0.500, 20)]
    sym_fine    = [round(x, 4) for x in np.linspace(0.005, 0.500, 20)]
    
    fll_fine = [0.005, 0.010, 0.020, 0.0314, 0.050, 0.080, 0.120]
    preamble_fine = [16, 24, 32, 48, 64]
    noise_fine = [0.0, 0.05, 0.10, 0.20, 0.35, 0.50]
    freq_offsets = [-0.030, -0.015, 0.0, +0.015, +0.030]
    time_offsets = [0.9995, 0.9998, 1.0, 1.0002, 1.0005]
    
    # Slice 1: High-Density 2D Matrix (Costas 0.005..0.500 x Sym 0.005..0.500 x Mod x Noise)
    grid_1 = list(itertools.product(
        modulations, [0.0314], costas_fine, sym_fine, [32], [0.0, 0.15], [0.0], [1.0], [10], [64]
    ))
    
    # Slice 2: Fine FLL vs Frequency Offset Slice (FLL x FreqOffset x Costas x Mod)
    grid_2 = list(itertools.product(
        modulations, fll_fine, [0.030, 0.0628, 0.100], [0.045], [32], [0.02], freq_offsets, [1.0], [10], [64]
    ))
    
    # Slice 3: Fine Preamble & Sampling Clock Drift Slice (SymSync x Preamble x Noise x Drift x Mod)
    grid_3 = list(itertools.product(
        modulations, [0.0314], [0.0628], [0.025, 0.045, 0.080], preamble_fine, noise_fine, [0.005], time_offsets, [10], [64]
    ))
    
    combined_grid = list(set(grid_1 + grid_2 + grid_3))
    total_trials = len(combined_grid)
    workers = min(cpu_count(), 8)
    
    print(f"Grid Configuration:")
    print(f"  -> TED Type                : TED_SIGNAL_TIMES_SLOPE_ML (y·y' ML TED)")
    print(f"  -> Costas Bandwidth Range  : 0.005 to 0.500 (Step 0.005)")
    print(f"  -> Symbol Sync Range       : 0.005 to 0.500 (Step 0.005)")
    print(f"  -> Frequency Offsets       : {freq_offsets}")
    print(f"  -> Sampling Clock Drifts   : {time_offsets}")
    print(f"  -> Noise Voltages (Vn)     : {noise_fine}")
    print(f"Total Combinatorial Executions: {total_trials:,} across {workers} CPU workers...")
    
    t_start = time.time()
    with Pool(processes=workers) as pool:
        all_results = pool.map(evaluate_fine_trial, combined_grid)
    ...
    # Save raw JSON
    json_path = os.path.join(output_dir, "fine_original_sweep_results.json")
    with open(json_path, "w") as f:
        json.dump(all_results, f, indent=2)
        
    # Save CSV
    csv_path = os.path.join(output_dir, "fine_original_sweep_results.csv")
    ...
    with open(os.path.join(output_dir, "fine_pinpoint_optimal_parameters.json"), "w") as f:
        json.dump(optimal_pinpoint, f, indent=2)
        
    print("\n================================================================================")
    print("            FINE PINPOINTED OPTIMAL VALUES SUMMARY (y·y' TED)                   ")
    print("================================================================================")
    for mod in modulations:

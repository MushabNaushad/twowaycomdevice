...
def run_hardware_optimization(hw_type='pluto', uri='ip:192.168.2.1', cf=433.98e6, tx_gain=20.0, rx_gain=30.0, candidates_file=None, top_n=None):
    results_dir = os.path.join(os.path.dirname(__file__), "results")
    os.makedirs(results_dir, exist_ok=True)
    
    print("================================================================================")
    print(f"       PHY2 SDR HARDWARE LIVE OPTIMIZATION ENGINE ({hw_type.upper()})           ")
    print("================================================================================")
    
    modulations = ['BPSK', 'QPSK']
    
    default_candidates_path = os.path.join(os.path.dirname(__file__), "top_50000_low_ber_candidates.json")
    ...
    if candidates_file and not os.path.exists(candidates_file):
        print(f"\033[93m[WARNING] Candidates file not found: {candidates_file}. Falling back to dense grid.\033[0m")
        candidates_file = None

    if candidates_file and os.path.exists(candidates_file):
        print(f"Loading top lowest-BER candidates from: {candidates_file} ...")
        with open(candidates_file, 'r') as f:
            cands = json.load(f)
        if top_n is not None and top_n > 0:
            cands = cands[:top_n]
        grid = [(c['mod_type'], float(c['fll_bw']), float(c['costas_bw']), float(c['sym_bw']), int(c['preamble_size'])) for c in cands]
    ...
    sim_rate = len(all_results) / max(elapsed, 0.001)
    
    print("\n" + "=" * 80)
    print(f" \033[92m\033[1m>>> HARDWARE SDR EVALUATION COMPLETED SUCCESSFULLY! <<<\033[0m")
    print(f"  • \033[1mActual Ran Time\033[0m       : \033[96m\033[1m{elapsed:.2f} seconds ({time_str})\033[0m")
    print(f"  • \033[1mTotal SDR Trials\033[0m      : \033[93m\033[1m{len(all_results):,} configurations on {hw_type.upper()}\033[0m")
    print(f"  • \033[1mAverage Throughput\033[0m    : \033[92m{sim_rate:.1f} trials/second\033[0m")
    print(f"  • \033[1mFinished Timestamp\033[0m    : {time.strftime('%Y-%m-%d %H:%M:%S')}")
    print("=" * 80 + "\n")
    
    # Merge with existing Hardware JSON & CSV
    ...
    if os.path.exists(json_path):
        try:
            ...
            for r in existing_hw:
                key = (r['mod_type'], round(float(r['fll_bw']), 4), round(float(r['costas_bw']), 4), round(float(r['sym_bw']), 4), int(r.get('preamble_size', 32)), r.get('hw_type', 'pluto'))
                hw_records_map[key] = r
        except Exception:
            pass
    ...
    merged_hw = list(hw_records_map.values())
    
    with open(json_path, "w") as f:
        json.dump(merged_hw, f, indent=2)
    ...
    # Pinpoint optimal hardware parameters (from cumulative dataset)
    opt_hw = {}
    for mod in modulations:
        mod_runs = [r for r in merged_hw if r['mod_type'] == mod]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--top-n', type=int, default=None, help="Limit number of candidates to evaluate (e.g. 1000, 10000, 50000)")
    args = parser.parse_args()
    sys.exit(run_hardware_optimization(args.hw, args.uri, args.freq, args.tx_gain, args.rx_gain, candidates_file=args.candidates, top_n=args.top_n))

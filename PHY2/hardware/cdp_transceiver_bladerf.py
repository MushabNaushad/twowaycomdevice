...
# Title: PHY2 CDP BladeRF SDR Transceiver
# Author: mushab404
# Description: PHY2 Full Duplex BladeRF SDR BPSK Transceiver
...
class cdp_transceiver_bladerf(gr.top_block, Qt.QWidget):

    def __init__(self):
        gr.top_block.__init__(self, "PHY2 CDP BladeRF SDR Transceiver", catch_exceptions=True)
        Qt.QWidget.__init__(self)
        self.setWindowTitle("PHY2 CDP BladeRF SDR Transceiver")
        ...
        self.settings = Qt.QSettings("gnuradio/flowgraphs", "cdp_transceiver_bladerf")
        ...
        self.CH_GAIN_TX = CH_GAIN_TX = 20
        self.CH_GAIN_RX = CH_GAIN_RX = 30
        ...
        for c in range(1, 2):
            self.top_grid_layout.setColumnStretch(c, 1)
        self._SDR_CF_range = qtgui.Range(47e6, 6e9, 1e5, 433.98e6, 200)
        ...
        self.top_grid_layout.addWidget(self._SDR_CF_win, 0, 0, 1, 1)
        for r in range(0, 1):
            self.top_grid_layout.setRowStretch(r, 1)
        for c in range(0, 1):
            self.top_grid_layout.setColumnStretch(c, 1)
        self._CH_GAIN_TX_range = qtgui.Range(0, 60, 1, 20, 200)
        self._CH_GAIN_TX_win = qtgui.RangeWidget(self._CH_GAIN_TX_range, self.set_CH_GAIN_TX, "TX Gain (dB)", "counter_slider", float, QtCore.Qt.Horizontal)
        self.top_grid_layout.addWidget(self._CH_GAIN_TX_win, 1, 0, 1, 1)
        for r in range(1, 2):
            self.top_grid_layout.setRowStretch(r, 1)
        for c in range(0, 1):
            self.top_grid_layout.setColumnStretch(c, 1)
        self._CH_GAIN_RX_range = qtgui.Range(0, 60, 1, 30, 200)
        self._CH_GAIN_RX_win = qtgui.RangeWidget(self._CH_GAIN_RX_range, self.set_CH_GAIN_RX, "RX Gain (dB)", "counter_slider", float, QtCore.Qt.Horizontal)
        self.top_grid_layout.addWidget(self._CH_GAIN_RX_win, 1, 1, 1, 1)
        for r in range(1, 2):
            self.top_grid_layout.setRowStretch(r, 1)
        for c in range(1, 2):
            self.top_grid_layout.setColumnStretch(c, 1)
        ...
        self.soapy_bladerf_source_0.set_gain(0, min(max(CH_GAIN_RX, -1.0), 60.0))
        ...
        self.soapy_bladerf_sink_0 = soapy.sink(dev, "fc32", 1, "driver=bladerf",
                                  stream_args, tune_args, settings)
        self.soapy_bladerf_sink_0.set_sample_rate(0, samp_rate)
        self.soapy_bladerf_sink_0.set_bandwidth(0, 0.0)
        self.soapy_bladerf_sink_0.set_frequency(0, SDR_CF)
        self.soapy_bladerf_sink_0.set_frequency_correction(0, 0)
        self.soapy_bladerf_sink_0.set_gain(0, min(max(CH_GAIN_TX, 17.0), 73.0))
    ...
    def set_SDR_CF(self, SDR_CF):
        self.SDR_CF = SDR_CF
        self.soapy_bladerf_sink_0.set_frequency(0, self.SDR_CF)
        self.soapy_bladerf_source_0.set_frequency(0, self.SDR_CF)

    def get_CH_GAIN_TX(self):
        return self.CH_GAIN_TX

    def set_CH_GAIN_TX(self, CH_GAIN_TX):
        self.CH_GAIN_TX = CH_GAIN_TX
        self.soapy_bladerf_sink_0.set_gain(0, min(max(self.CH_GAIN_TX, 17.0), 73.0))

    def get_CH_GAIN_RX(self):
        return self.CH_GAIN_RX

    def set_CH_GAIN_RX(self, CH_GAIN_RX):
        self.CH_GAIN_RX = CH_GAIN_RX
        self.soapy_bladerf_source_0.set_gain(0, min(max(self.CH_GAIN_RX, -1.0), 60.0))
...
def main(top_block_cls=cdp_transceiver_bladerf, options=None):

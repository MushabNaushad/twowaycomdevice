...
# -*- coding: utf-8 -*-
"""
PHY2 Master Terminal Interactive Plotter & Dynamic Analyzer
Enables users to dynamically change parameters from the terminal:
- BER vs Symbol Sync with multiple Costas lines across the full FLL range (0.005..1.000 rad/sym).
- 2D Parametric Heatmaps (Costas vs Symbol Sync).
- Multi-parameter CSV inspection and instant CSV export.
"""
...
def render_terminal_ascii_plot(mod_type, fll_bw, selected_costas, records, width=80, height=22):
    ...
    all_fll = sorted(list(set(r['fll_bw'] for r in records)))
    
    # Find closest available FLL
    closest_fll = min(all_fll, key=lambda f: abs(f - fll_bw))
    ...
    if not series_data:
        print(f"{C_RED}[WARN] No matching simulation records for FLL={closest_fll:.4f} rad/sym with selected Costas values.{C_RESET}")
        return
    ...
    def map_x(x): return int(max(0, min(width - 1, ((x - x_min) / (x_max - x_min)) * (width - 1))))
    def map_y(y): return int(max(0, min(height - 1, ((y_max - y) / (y_max - y_min)) * (height - 1))))
    ...
    print(f" {C_YELLOW}FLL Band-Edge Loop BW = {closest_fll:.4f} rad/sym{C_RESET} | {len(series_data)} Costas Lines")
...
def render_terminal_heatmap(mod_type, fll_bw, records, metric='ber'):
    ...
    print(f" Metric: {C_YELLOW}{metric.upper()}{C_RESET} | Fixed FLL Band-Edge BW = {closest_fll:.4f} rad/sym")
...
def interactive_repl():
    ...
    print(f"{C_BOLD}{C_CYAN}================================================================================{C_RESET}")
    print(f"Loaded {len(records):,} high-density simulation trials spanning 0.005..1.000 rad/sym full range.")
    print(f"Available FLL loop bandwidth values: {all_fll}")
    print("\nCommands:")
    ...
    current_mod = "BPSK"
    current_fll = 0.0314
    current_costas_lines = [0.005, 0.010, 0.025, 0.0628, 0.135, 0.250, 0.500, 1.000]
    ...
    while True:
        ...
        if cmd in ['q', 'quit', 'exit']:
            ...
            if len(parts) > 1:
                ...
                print(f"Current FLL: {current_fll:.4f} rad/sym (Available: {all_fll})")
            ...
            print("─" * 86)
            closest_fll = min(all_fll, key=lambda f: abs(f - current_fll))
            matches = [r for r in records if r['mod_type'] == current_mod and abs(r['fll_bw'] - closest_fll) < 0.006]

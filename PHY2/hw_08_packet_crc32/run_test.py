...
# -*- coding: utf-8 -*-
"""
PHY2 Stage 08: Hardware Packet Engine & CRC32 Verification Test Runner (BPSK & QPSK)
Verifies tagged stream packet framing and IEEE 802.3 CRC32 across SDR Hardware (Pluto / BladeRF / RTL-SDR / Channel).
"""
...
class HardwarePacketEngineTester(gr.top_block):
    def __init__(self, test_payload, mod_type='BPSK', payload_size=64, packets=10, preamble_size=32,
        ...
        super().__init__("Hardware_Packet_Engine_Tester", catch_exceptions=True)
...
def run_test(mod_type='ALL', hw_type='sim', uri='ip:192.168.2.1'):
    print("================================================================================")
    print(f" [PHY2 Stage 08] Hardware Packet Engine & CRC32 Verification (HW: {hw_type.upper()}) ")
    print("================================================================================")
    ...
    for mod in modulations:
        ...
        print(f" Transmitted Packets : {packets} | Received Packets: {received_pkts} | CRC Valid: {matched_pkts}")
        print(f" Packet Delivery Ratio: {pdr:.1f}%")
        
        if received_pkts >= (packets - 1) and matched_pkts == received_pkts:
            print(f" -> [PASS] Hardware Packet Engine & CRC32 Integrity Verified for {mod}!")
        else:
            print(f" -> [FAIL] CRC failures or packet drop detected.")
            all_passed = False
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Stage 08 Hardware Test Runner")

...
# -*- coding: utf-8 -*-
"""
PHY2 Stage 09: Hardware CDP Transceiver Architecture Test
...
class Stage09CDPTransceiver(gr.top_block):
    def __init__(self, test_payload, mod_type='BPSK', payload_size=64, packets=5, hw_type='sim', uri='ip:192.168.2.1', cf=433.98e6, gain=20.0):
        super().__init__("Stage09_CDP_Transceiver", catch_exceptions=True)
        ...
        if hw_type in ['pluto', 'bladerf', 'rtlsdr']:
            ...
            self.channel = channels.channel_model(
                noise_voltage=0.02, frequency_offset=0.005, epsilon=1.0001, taps=[1.0, 0.12]
            )
            self.use_rf = False
...
def run_stage_09(hw_type='sim', mod_type='ALL', uri='ip:192.168.2.1', freq=433.98e6, gain=20.0):
    mods = ['BPSK', 'QPSK'] if mod_type == 'ALL' else [mod_type]
    all_passed = True
    payload = [int((i * 37) % 256) for i in range(320)]
    
    for m in mods:
        tb = Stage09CDPTransceiver(test_payload=payload, mod_type=m, hw_type=hw_type, uri=uri, cf=freq, gain=gain)
        tb.run()
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Stage 09 Hardware CDP Transceiver Test Runner")

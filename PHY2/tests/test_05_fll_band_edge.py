...
# -*- coding: utf-8 -*-
"""
PHY2 Stage 05: Hardware FLL Band-Edge Frequency Recovery Test
...
class Stage05FLLTransceiver(gr.top_block):
    def __init__(self, test_payload, mod_type='BPSK', sps=4, alpha=0.35, fll_bw=0.0314, freq_offset=0.02, hw_type='sim', uri='ip:192.168.2.1', cf=433.98e6, gain=20.0):
        super().__init__("Stage05_FLL_Transceiver", catch_exceptions=True)
        ...
        if hw_type in ['pluto', 'bladerf', 'rtlsdr']:
            ...
            self.channel = channels.channel_model(
                noise_voltage=0.01, frequency_offset=freq_offset, epsilon=1.0, taps=[1.0]
            )
            self.use_rf = False
        ...
        if self.use_rf:
            ...
            self.connect(self.hw_src, self.agc, self.fll, self.rx_filter, self.symbol_sync, self.costas, self.decoder, self.sink)
        else:
...
def run_stage_05(hw_type='sim', mod_type='ALL', uri='ip:192.168.2.1', freq=433.98e6, gain=20.0):
    mods = ['BPSK', 'QPSK'] if mod_type == 'ALL' else [mod_type]
    all_passed = True
    payload = [int((i * 17) % 256) for i in range(1000)]
    
    for m in mods:
        tb = Stage05FLLTransceiver(test_payload=payload, mod_type=m, hw_type=hw_type, uri=uri, cf=freq, gain=gain)
        tb.run()
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Stage 05 Hardware FLL Test Runner")

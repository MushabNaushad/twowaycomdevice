...
# -*- coding: utf-8 -*-
"""
PHY2 Stage 08: Hardware Packet Engine & CRC32 Verification Test
...
class Stage08PacketTransceiver(gr.top_block):
    def __init__(self, test_payload, mod_type='BPSK', payload_size=64, packets=5, hw_type='sim', uri='ip:192.168.2.1', cf=433.98e6, gain=20.0):
        super().__init__("Stage08_Packet_Transceiver", catch_exceptions=True)
...
def run_stage_08(hw_type='sim', mod_type='ALL', uri='ip:192.168.2.1', freq=433.98e6, gain=20.0):
    mods = ['BPSK', 'QPSK'] if mod_type == 'ALL' else [mod_type]
    all_passed = True
    payload = [int((i * 31) % 256) for i in range(320)]
    
    for m in mods:
        tb = Stage08PacketTransceiver(test_payload=payload, mod_type=m, hw_type=hw_type, uri=uri, cf=freq, gain=gain)
        tb.run()
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Stage 08 Hardware Packet Engine Test Runner")

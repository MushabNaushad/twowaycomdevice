...
# -*- coding: utf-8 -*-
"""
PHY2 Stage 04: Costas Loop Carrier Phase Recovery Test Runner (BPSK & QPSK)
Verifies carrier frequency/phase acquisition and phase ambiguity resolution across BPSK and QPSK.
"""
...
class CostasTester(gr.top_block):
    def __init__(self, test_bytes, mod_type='BPSK', freq_offset=0.005, noise_volt=0.01,
                 costas_bw=0.0628, sym_bw=0.045, sps=4, alpha=0.35, samp_rate=32000):
        super().__init__("Costas_Tester", catch_exceptions=True)
        ...
        # Channel with carrier frequency offset and noise
...
def evaluate_costas(rx_bits, tx_bits, discard_transient=120):
    ...
    if len(rx_b) < len(tx_b) + discard_transient:
        return 1.0, len(tx_b), len(tx_b), 0
    ...
    if len(aligned) == len(tx_b):
        ...
        return ber, int(errs), len(steady_tx), 1
        
    return 1.0, len(tx_b), len(tx_b), 0

def run_test(mod_type='ALL'):
    print("================================================================================")
    print(" [PHY2 Stage 04] Costas Loop Carrier Phase Recovery Test (BPSK & QPSK)          ")
    print("================================================================================")
    
    modulations = ['BPSK', 'QPSK'] if mod_type.upper() == 'ALL' else [mod_type.upper()]
    np.random.seed(42)
    test_bytes = [0x55] * 20 + list(np.random.randint(0, 256, 250))
    
    test_offsets = [
        ("Moderate Frequency Offset (+0.5% samp_rate)", +0.005),
        ("Moderate Negative Offset (-0.5% samp_rate)",  -0.005),
        ("Slight Frequency Offset (+0.2% samp_rate)",    +0.002),
    ]
    ...
    for mod in modulations:
        ...
        for desc, fo in test_offsets:
            tb = CostasTester(test_bytes, mod_type=mod, freq_offset=fo, noise_volt=0.01)
            tb.run()
            ber, errs, total, pol = evaluate_costas(tb.sink.data(), tx_bits)
            
            print(f"  {desc}")
            print(f"    Steady-State BER: {ber:.6f} ({errs}/{total} errors)")
            
            if ber < 0.05:
                print(f"    -> [PASS] Costas loop locked carrier phase.")
            else:
                print(f"    -> [FAIL] Costas loop phase lock failed!")
                all_passed = False
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Stage 04 Test Runner")

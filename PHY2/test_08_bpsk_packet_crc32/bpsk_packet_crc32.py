...
# Title: PHY2 Test 08: BPSK Packet Engine & CRC32
# Author: mushab404
# Description: PHY2 Stage 08: Full BPSK Packet Engine with Protocol Formatter & CRC32
...
class bpsk_packet_crc32(gr.top_block):

    def __init__(self):
        gr.top_block.__init__(self, "PHY2 Test 08: BPSK Packet Engine & CRC32", catch_exceptions=True)
        ...
        self.payload_size = payload_size = 32
        self.num_packets = num_packets = 5
        ...
        self.channels_channel_model_0 = channels.channel_model(
            ...
            noise_seed=0,
            block_tags=False)
        self.blocks_vector_source_x_preamble = blocks.vector_source_b([0x55] * preamble_len * num_packets, False, 1, [])
        self.blocks_vector_source_x_0 = blocks.vector_source_b([int((p*13 + i) % 256) for p in range(num_packets) for i in range(payload_size)], False, 1, [])
        self.blocks_vector_sink_x_0 = blocks.vector_sink_b(1, 1024)
        self.blocks_tagged_stream_mux_0 = blocks.tagged_stream_mux(gr.sizeof_char*1, "packet_len", 0)
        self.blocks_stream_to_tagged_stream_preamble = blocks.stream_to_tagged_stream(gr.sizeof_char, 1, preamble_len, "packet_len")
    ...
    def set_preamble_len(self, preamble_len):
        ...
        self.blocks_vector_source_x_preamble.set_data([0x55] * self.preamble_len * self.num_packets, [])
        self.blocks_stream_to_tagged_stream_preamble.set_packet_len(self.preamble_len)
        self.blocks_stream_to_tagged_stream_preamble.set_packet_len_pmt(self.preamble_len)
    ...
    def set_payload_size(self, payload_size):
        ...
        self.blocks_vector_source_x_0.set_data([int((p*13 + i) % 256) for p in range(self.num_packets) for i in range(self.payload_size)], [])
...
def main(top_block_cls=bpsk_packet_crc32, options=None):
    tb = top_block_cls()
